...
class CatalogueViewModel : ViewModel() {
    ...
    fun getMoviesList(inputStream: InputStream): List<Movie> {
        moviesList = DataParser.getMoviesList(RawReader.readFromRaw(inputStream)).shuffled()
        return moviesList
    }
    ...
    fun getShowsList(inputStream: InputStream): List<Show> {
        showsList = DataParser.getShowsList(RawReader.readFromRaw(inputStream)).shuffled()
        return showsList
    }
}